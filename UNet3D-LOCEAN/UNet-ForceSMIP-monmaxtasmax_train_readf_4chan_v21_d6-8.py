...
ncvar='monmaxtasmax'  # CMIP variable name to be used
...
class Dataset(torch.utils.data.Dataset):
  ...
  def __getitem__(self, index):
        ...
        std_monmaxtasmax=torch.load(outdir+'X_std_tensor_'+list_std+'monmaxtasmax_v3.pt')
        X = torch.stack(
            (torch.load(outdir+'X_norm_tensor_'+X_ID+'tas_v3.pt')/std_tas,
             ...
             torch.load(outdir+'X_norm_tensor_'+X_ID+'monmaxtasmax_v3.pt')/std_monmaxtasmax,
        ...
        y = torch.stack(
            (torch.load(outdir+'y-EM_norm_tensor_'+y_ID+ncvar+'_v3.pt')/std_em,
             torch.load(outdir+'y-ANO_norm_tensor_'+X_ID+ncvar+'_v3.pt')/std_ano),
            dim=0)
...
class UNet(nn.Module):
    def __init__(self,thick,dr,k):
        ...
        self.conv1 = double_conv(5, thick,dr,k,0)  # 876,72,144
        ...
        self.convg = conv_geo_lin(thick, 2,k,0)
...
class MSE_areaweighted_Loss(nn.Module):
    def __init__(self, weights,stdX,std_y_ANO,std_y_EM,alpha=0):
        super(MSE_areaweighted_Loss, self).__init__()
        self.weights = weights
        self.stdX = stdX
        self.std_y_ANO = std_y_ANO
        self.std_y_EM = std_y_EM
        self.alpha = alpha
        
    def forward(self, inputs, y_hat, targets):
        error = torch.square(y_hat - targets)
        weighted_error = error * self.weights
        non_add=torch.square(self.stdX*inputs[:,0,:,:,:]
                            -(y_hat[:,0,:,:,:]*self.std_y_EM
                            +y_hat[:,1,:,:,:]*self.std_y_ANO))
        non_add_t=torch.mean(non_add * self.weights)
        loss = torch.mean(weighted_error) + self.alpha*non_add_t
        rmse_non_add=torch.sqrt(non_add_t)
        rmse_ex = torch.sqrt(torch.mean(weighted_error[:,0,:,:,:]))*self.std_y_EM
        rmse_in = torch.sqrt(torch.mean(weighted_error[:,1,:,:,:]))*self.std_y_ANO
        return loss,rmse_non_add,rmse_ex,rmse_in
...
criterion = MSE_areaweighted_Loss(wgts,stdX,std_y_ano,std_y_em,0)
...
for epoch in range(epoch_prev,NB_epoch+epoch_prev+1):  # loop over the dataset multiple times
    ...
    for i, data in enumerate(training_generator, 0):
        ...
        # gradient optimize
        loss, rmse_nadd, rmse_ex,rmse_in = criterion(inputs,outputs,labels)
        ...
        running_rmse_ex += rmse_ex.item()
        running_rmse_in += rmse_in.item()
        running_rmse_nadd += rmse_nadd.item()
    
    # test
    correct, total = 0, 0
    test_running_loss = 0.0
    test_running_rmse_ex = 0.0
    test_running_rmse_in = 0.0
    test_running_rmse_nadd = 0.0
    ...
    with torch.no_grad():
      for j, data in enumerate(validation_generator, 0):
         ...
         outputs = model(inputs)
         # get the predictions  
         t_loss, t_rmse_nadd, t_rmse_ex,t_rmse_in = criterion(inputs,
                                                    outputs, labels) 
         test_running_loss += t_loss
         test_running_rmse_nadd += t_rmse_nadd
         test_running_rmse_ex += t_rmse_ex
         test_running_rmse_in += t_rmse_in

    loss_test[0,epoch]=test_running_loss / (j+1)
    loss_train[0,epoch]=running_loss / (i+1)
    loss_test[1,epoch]=test_running_rmse_ex / (j+1)
    loss_train[1,epoch]=running_rmse_ex / (i+1)
    loss_test[2,epoch]=test_running_rmse_in / (j+1)
    loss_train[2,epoch]=running_rmse_in / (i+1)
    loss_test[3,epoch]=test_running_rmse_nadd / (j+1)
    loss_train[3,epoch]=running_rmse_nadd / (i+1)
